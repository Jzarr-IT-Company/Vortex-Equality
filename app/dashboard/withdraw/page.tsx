...
import { FinancialWallet, Wallet, Profile } from '@/lib/types'
...
export default function WithdrawPage() {
  ...
  const [financialWallet, setFinancialWallet] = useState<FinancialWallet | null>(null)
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: savedCryptoWallet } = await supabase
      .from('user_crypto_wallets')
      .select('*')
      .eq('user_id', user.id)
      .maybeSingle()

    if (savedCryptoWallet?.wallet_address) {
      setCryptoAddress(savedCryptoWallet.wallet_address)
    }
    if (savedCryptoWallet?.coin) {
      const savedCoin = String(savedCryptoWallet.coin).toLowerCase()
      if (savedCoin.includes('trc20') || savedCoin.includes('tron') || savedCoin.includes('trx')) setSelectedNetwork('trc20')
      else setSelectedNetwork('bep20')
    }
    ...
    if (walletData) {
      setFinancialWallet(walletData)
      setAssetWallet({
        id: user.id,
        user_id: user.id,
        wallet_type: 'asset',
        balance: Number(walletData.main_balance || 0) + Number(walletData.active_deposit || 0),
        total_profit_earned: Number(walletData.total_claimed_profit || 0),
        initial_capital: Number(walletData.active_deposit || 0),
        cap_reached: Boolean(walletData.is_maxed_out),
      ...
      setBonusWallet({
        ...
        user_id: user.id,
        wallet_type: 'bonus',
        balance: Number(walletData.network_bonus_balance || 0),
  ...
  const getFeePercentage = () => {
    if (selectedWallet === 'bonus') return 5
    if (financialWallet) return financialWallet.is_bep_reached ? 5 : 20
    return getRoiPercentage() >= 100 ? 5 : 20
  }
  ...
  const handleWithdraw = async () => {
    ...
    if (!/^\d{6}$/.test(withdrawalPin)) {
      setError('Please enter your 6-digit withdrawal PIN')
      return
    }
        ...
        <Card className="border-blue-500/30 bg-slate-900">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-3 rounded-xl bg-blue-500/10 p-4">
              ...
              <p className="text-slate-400">Admin approval triggers the Plisio payout.</p>
              </div>
            </div>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-slate-400">Submit a request to your saved USDT wallet. Admin approval sends the payout through Plisio.</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 p-3">
          ...
          <div><p className="text-xs font-medium text-white">Admin Review</p><p className="text-[10px] text-slate-500">Plisio payout</p></div>
        </div>
        <div className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 p-3">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <button
              ...
              className={cn(
                'relative flex flex-col gap-4 overflow-hidden rounded-lg p-4 text-left transition-colors sm:p-5',
            ...
            >
              <div className="flex items-start justify-between w-full">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    "rounded-full p-2.5",
                    selectedWallet === 'asset' ? "bg-white/20" : "bg-muted"
                  )}>
              ...
              <div className={cn(
                ...
                <div className="flex justify-between items-center">
                    <span>BEP status:</span>
                  <span className="font-semibold">{financialWallet?.is_bep_reached ? 'Reached' : 'Not reached'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span>Admin fee:</span>
                  <span className="font-semibold">{getFeePercentage()}%</span>
                </div>
                {!financialWallet?.is_bep_reached && (
                  <p className={cn("text-[10px] mt-2", selectedWallet === 'asset' ? "text-blue-200/70" : "text-[#55555A]")}>
                    Fee drops to 5% after total withdrawals reach your active asset
                  </p>
                )}
              </div>
            </button>
            ...
            >
              <div className="flex items-start justify-between w-full">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    "rounded-full p-2.5",
                    selectedWallet === 'bonus' ? "bg-white/20" : "bg-muted"
                  )}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {networkOptions.map((net) => (
                <button
                  ...
                  className={cn(
                    'flex min-h-28 flex-col items-center justify-center rounded-lg border p-3 text-center transition-colors sm:p-4',
                ...
                >
                  <div className="bg-white p-2 rounded-full mb-2 relative shadow-sm">
                    ...
                    {selectedNetwork === net.id && (
                      <div className="absolute -right-1 -top-1 rounded-full border-2 border-card bg-emerald-500 p-0.5">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                type={showCryptoAddress ? 'text' : 'password'}
                placeholder="Enter your USDT wallet address"
                value={cryptoAddress}
                onChange={(e) => { setCryptoAddress(e.target.value); setError(null) }}
                className="border-slate-700 bg-slate-800 pr-11 font-mono text-sm text-white"
              />
              <Button
                type="button"
                variant="ghost"
                ...
                onClick={() => setShowCryptoAddress((value) => !value)}
              >
                {showCryptoAddress ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </Button>
            </div>
            <p className="text-xs text-slate-500">Wallet addresses are saved per user. Multiple users may use the same address.</p>
          </div>

          {/* Withdrawal PIN */}
          <div className="space-y-2">
            <Label className="text-slate-300">6-Digit Withdrawal PIN</Label>
            <div className="relative">
              <Input
                ...
                value={withdrawalPin}
                onChange={(e) => { setWithdrawalPin(e.target.value.replace(/\D/g, '').slice(0, 6)); setError(null) }}
                className="h-12 border-slate-700 bg-slate-800 pr-11 text-center font-mono text-xl tracking-[0.4em] text-white placeholder:text-sm placeholder:tracking-normal"
                maxLength={6}
              />
              <Button
                type="button"
                ...
                size="icon"
                className="absolute right-1 top-1/2 h-8 w-8 -translate-y-1/2 text-slate-400 hover:text-white"
                onClick={() => setShowWithdrawalPin((value) => !value)}
              >
            ...
            <p className="flex items-center gap-2 text-xs text-slate-500">
              ...
              Required before any balance is deducted.
            </p>
          </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="text-sm">
                <p className="font-semibold text-primary">
                  ...
                  {selectedWallet === 'asset' && !financialWallet?.is_bep_reached && ' (BEP not reached)'}
                  {selectedWallet === 'asset' && financialWallet?.is_bep_reached && ' (BEP reached)'}
                  {selectedWallet === 'bonus' && ' (Bonus Wallet)'}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  {selectedWallet === 'asset'
                    ? 'Main + Active Asset withdrawals use 20% before BEP and 5% after BEP. Capital withdrawals reduce your active asset.'
          ...
          <Button
            ...
            disabled={submitting || !amount || parseFloat(amount) < MIN_WITHDRAWAL || !cryptoAddress || withdrawalPin.length !== 6}
          >
            {submitting
          ...
          <p className="text-center text-xs text-slate-500">
            Requests stay pending until an admin approves the Plisio payout. Funds are deducted from your wallet immediately.
          </p>
        </CardContent>
      </Card>
    </div>
